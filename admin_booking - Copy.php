...
if($act==='save_booking'){
    ...
    if(!$nama||!$email){echo json_encode(['ok'=>false,'msg'=>'Nama & email wajib']);exit;}
    ...
    $sisa=max(0,$tinv-$dp);
    if($bid){
        $s=$db->prepare("UPDATE booking SET nama=?,email=?,whatsapp=?,tanggal=?,waktu_kegiatan=?,catatan=?,total_harga=?,discount=?,total_invoice=?,dp=?,sisa_bayar=?,status_bayar=?,updated_at=NOW() WHERE id=?");
        $s->bind_param("ssssssiiiissi",$nama,$email,$wa,$tgl,$waktu,$catatan,$total,$disc,$tinv,$dp,$sisa,$status,$bid);
        $s->execute();
        $db->query("DELETE FROM booking_items WHERE booking_id=$bid");
    } else {
        $no='PNT'.date('ymd').str_pad(rand(1,9999),4,'0',STR_PAD_LEFT);
        $src='admin';
        $s=$db->prepare("INSERT INTO booking (no_invoice,nama,email,whatsapp,tanggal,waktu_kegiatan,catatan,sumber,total_harga,discount,total_invoice,dp,sisa_bayar,status_bayar) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $s->bind_param("ssssssssiiiiis",$no,$nama,$email,$wa,$tgl,$waktu,$catatan,$src,$total,$disc,$tinv,$dp,$sisa,$status);
        $s->execute();
        $bid=$db->insert_id;
    }
...
if($act==='export_excel'){
    ...
    $wh="WHERE 1=1";
    if($fs) $wh.=" AND b.status_bayar='$fs'";
    if($fq) $wh.=" AND (b.nama LIKE '%$fq%' OR b.no_invoice LIKE '%$fq%' OR b.whatsapp LIKE '%$fq%' OR b.email LIKE '%$fq%')";
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="booking_'.date('Ymd').'.xls"');
    echo '<table border="1"><thead><tr style="background:#0059b3;color:#fff">
    <th>No Invoice</th><th>Nama</th><th>Email</th><th>WhatsApp</th><th>Medsos</th>
    <th>Tgl Trip</th><th>Waktu</th><th>Item</th>
    <th>Total</th><th>Diskon</th><th>Total Invoice</th><th>DP</th><th>Sisa</th>
    <th>Metode Bayar</th><th>Status</th><th>Sumber</th><th>Tgl Booking</th>
    </tr></thead><tbody>';
    $res=$db->query("SELECT b.*,GROUP_CONCAT(CONCAT(bi.produk,' x',bi.qty) SEPARATOR ', ') ilist FROM booking b LEFT JOIN booking_items bi ON bi.booking_id=b.id $wh GROUP BY b.id ORDER BY b.created_at DESC");
    if($res) while($r=$res->fetch_assoc()){
        echo "<tr>
        <td>{$r['no_invoice']}</td><td>{$r['nama']}</td><td>{$r['email']}</td>
        <td>{$r['whatsapp']}</td><td>{$r['medsos']}</td>
        <td>{$r['tanggal']}</td><td>{$r['waktu_kegiatan']}</td><td>{$r['ilist']}</td>
        <td>".number_format($r['total_harga']??0,0)."</td>
        <td>".number_format($r['discount']??0,0)."</td>
        <td>".number_format($r['total_invoice']??0,0)."</td>
        <td>".number_format($r['dp']??0,0)."</td>
        <td>".number_format($r['sisa_bayar']??0,0)."</td>
        <td>{$r['status_bayar']}</td><td>{$r['sumber']}</td><td>{$r['created_at']}</td></tr>";
    }
...
if($fq) $wh.=" AND (nama LIKE '%$fq%' OR no_invoice LIKE '%$fq%' OR whatsapp LIKE '%$fq%' OR email LIKE '%$fq%')";
...
<div class="mo" id="moBk"><div class="md" style="max-width:760px">
  ...
  <div class="mb">
    ...
    <div class="fgrid">
      ...
      <div class="fg">
        <label>Email *</label>
        <input id="bkEmail" type="email" placeholder="contoh@gmail.com" oninput="valEmail(this)" autocomplete="off">
...
<div class="tw">
  <div class="th-bar">
    <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      <input class="si" name="q" placeholder="Cari nama / invoice / WA / email..." value="<?= htmlspecialchars($fq) ?>">
  ...
  <table>
    ...
    <tr>
      ...
      <td>
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:2px"><?= htmlspecialchars($b['email']??'') ?></div>
...
function valEmail(input){
  ...
  if(!v){h.style.display='none';input.style.borderColor='';return;}
...
function openNewBk(){
  ...
  ['bkNama','bkEmail','bkWA','bkCat','bkWaktu'].forEach(function(i){var el=document.getElementById(i);if(el){el.value='';el.style.borderColor='';}});
...
async function openEditBk(id){
  ...
  document.getElementById('bkNama').value=d.nama;document.getElementById('bkEmail').value=d.email;document.getElementById('bkWA').value=d.whatsapp;
...
async function saveBk(){
  ...
  var body=new URLSearchParams({act:'save_booking',bid:document.getElementById('bkId').value||0,nama:nama,email:email,whatsapp:document.getElementById('bkWA').value,tanggal:document.getElementById('bkTgl').value,waktu:document.getElementById('bkWaktu').value,catatan:document.getElementById('bkCat').value,discount:document.getElementById('bkDisc').value||0,dp:document.getElementById('bkDP').value||0,status:document.getElementById('bkSts').value,items:JSON.stringify(items)});
...
async function openDetail(id){
  ...
  document.getElementById('moDetB').innerHTML=
    ...
    +'<div style="font-size:12px;line-height:2.1"><strong style="color:#fff;font-size:14px">'+d.nama+'</strong><br><span style="color:var(--cyan)">'+d.email+'</span><br>'+(d.whatsapp||'&mdash;')+'<br><span style="color:var(--muted)">'+(d.medsos||'&mdash;')+'</span></div></div>'
...
function buildInvHTML(d, items){
  ...
  var sisa  = parseInt(d.sisa_bayar   || 0) || Math.max(0, tinv - dp);
  ...
  return '<div id="inv-print-area" style="padding:44px 52px 56px;font-family:Georgia,serif;background:#fff;color:#333;font-size:13px">'
    ...
    +'<tr><td style="color:#777;padding:5px 0;font-family:Georgia,serif;font-size:13px">E-Mail</td><td style="color:#777;padding:5px 8px">:</td><td style="color:#222;padding:5px 0;font-family:Georgia,serif;font-size:13px">'+(d.email||'—')+'</td></tr>'
    ...
    +'<tr><td style="color:#777;padding:4px 0;font-family:Georgia,serif;font-size:13px">Date</td><td style="color:#777;padding:4px 8px">:</td><td style="color:#222;padding:4px 0;font-family:Georgia,serif;font-size:13px">'+tgl+'</td></tr>'
    +'<tr><td style="color:#777;padding:4px 0;font-family:Georgia,serif;font-size:13px">Payment Date</td><td style="color:#777;padding:4px 8px">:</td><td style="color:#222;padding:4px 0;font-family:Georgia,serif;font-size:13px">'+(d.tanggal||'—')+'</td></tr>'
    +'</table></div></div>'
    ...
    +'<tr><td style="width:55%;padding:0"></td><td style="padding:10px 0;font-weight:700;font-size:13px;color:#000;font-family:Georgia,serif">Ammount Paid (DP)</td><td style="padding:10px 0;font-weight:700;font-size:13px;color:#000;font-family:Georgia,serif;text-align:right">'+(dp>0?fD(dp):'—')+'</td></tr>'
...
async function genPDF(d, itemsArg){
  ...
  var billRows = [['Name', d.nama||'—'],['E-Mail', d.email||'—'],['WhatsApp', d.whatsapp||'—']];
  doc.setFontSize(9);
  ...
  var payRows = [
    ...
    ['Date',        tgl,                 [30,30,30]],
    ['Payment Date',d.tanggal||'—',      [30,30,30]],
  ];
  doc.setFontSize(9);
  ...
  var sumRows = [
    ...
    ['Ammount Paid (DP)',    dp>0 ? fD(dp) : '—',     false, false],
